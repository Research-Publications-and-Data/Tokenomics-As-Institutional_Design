...
gov = pd.read_csv(_PROJ / 'data' / 'processed' / 'governance_concentration_april2026.csv')
token_chain_map = dict(zip(gov['token'], gov['chain']))
gov_tokens = set(gov['token'])

exclusions = pd.read_csv(_PROJ / 'data' / 'processed' / 'exclusions_log.csv')
...
TOKEN_FILE_MAP = {t: str(_PROJ / 'data' / 'raw' / 'holder_lists' / f'{t}_holders.csv') for t in gov_tokens}
...
top10_df.to_csv(_PROJ / 'data' / 'processed' / 'insider_classification.csv', index=False)
print(f"\nSaved: data/processed/insider_classification.csv")
