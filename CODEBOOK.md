...
**Sector-member variant B (pending v1.0.3).** `exhibits/romano_wolf_stepdown_sector_mw.py` re-runs the same family-wise correction with the sector member set to the paper's headline balanced DePIN-versus-DeFi Mann-Whitney on the pass-through HHI (Cohen's d = 0.65, p = 0.028, N = 15 and 15; reads `../b2/paper/analysis_n52_2026-05-29/sector_contrast_hhi_vectors_2026-06-02.csv`), and writes `exhibits/romano_wolf_results_sector_mw.json`. It imports the variant-A core so the two cannot drift. Under variant B the sector contrast sits just above the family-wise threshold (Romano-Wolf p approximately 0.05; HALT-2 flags it as crossing 0.05 under correction, by design), insider retention carries the correction cleanly (p = 0.019), and the Livepeer-inclusive subsidy is at the boundary (p = 0.049). Variant A (the omnibus Kruskal-Wallis above) and variant B together support the of-record reading: insider retention survives a joint four-test family-wise correction under either sector specification; the sector contrast is a marginal, specification-dependent medium effect.
