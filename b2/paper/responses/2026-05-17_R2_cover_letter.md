...
**Cascade through dependent statistics is contained.** DePIN sector mean shifts from 0.077 to 0.071; the DePIN-vs-DeFi Mann-Whitney sector contrast remains significant (p = 0.020 from 0.018; Cohen's d = 0.94 from 0.98, still large effect by Cohen's threshold); LOO 30-of-30 significant-iteration robustness is preserved; the Synergy-HHI Pearson correlation remains statistically significant (r = -0.32, p = 0.047); the Livepeer-driven subsidy interpretation is reinforced (Spec 4 subsidy p = 0.88 absent Livepeer). All headline findings are qualitatively preserved.
