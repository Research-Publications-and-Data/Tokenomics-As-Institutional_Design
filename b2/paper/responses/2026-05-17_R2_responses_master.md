...
The most consequential single result of the R1→R2 reproduce-and-sync is a substantive interpretive change in Section 3.5: with corrected holding HHI baselines for Uniswap and Optimism, both protocols flip from delegation-mediated dispersion to delegation amplification. The R2 manuscript now documents a **universal-amplification finding** across all eight protocols in the Table 7 sample (range 1.9x to 6.8x; mean 4.1x), with a robustness check under symmetric PCA-exclusion methodology confirming the finding holds across all 5 Tally-sourced protocols even when foundation and aggregation-contract delegates are excluded from voting HHI (Section 3.7).
...
**Table 7 before/after:**

| Protocol  | Sector | Old Holding | **New Holding** | Voting HHI | Old Ratio | **New Ratio** | Direction         |
|-----------|--------|-------------|-----------------|------------|-----------|---------------|-------------------|
| DIMO      | DePIN  | 0.038       | 0.038           | 0.228      | 6.0x      | 6.0x          | unchanged         |
| Lido      | DeFi   | 0.018       | **0.013**       | 0.088      | 4.8x      | **6.8x**      | amplifies (higher)|
| WeatherXM | DePIN  | 0.148       | 0.148           | 0.486      | 3.3x      | 3.3x          | unchanged         |
| Compound  | DeFi   | 0.028       | 0.028           | 0.053      | 1.9x      | 1.9x          | unchanged         |
| Aave      | DeFi   | 0.020       | 0.020           | 0.076      | 3.8x      | 3.8x          | unchanged         |
| Uniswap   | DeFi   | 0.032       | **0.010**       | 0.027      | 0.84x     | **2.7x**      | **FLIP**          |
| Arbitrum  | Infra  | 0.012       | 0.012           | 0.052      | 4.3x      | 4.3x          | unchanged         |
| Optimism  | Infra  | 0.042       | **0.009**       | 0.033      | 0.79x     | **3.7x**      | **FLIP**          |

**Substantive interpretive consequence.** The flip is not a typo fix: it changes the Section 3.5 finding from "delegation amplifies unevenly, with two infrastructure protocols showing opposite patterns" to "delegation universally amplifies governance concentration across all eight protocols sampled, with magnitude (not direction) varying by institutional design within sector." We have rewritten Section 3.5 (opening paragraph, DeFi paragraph, Optimism/Arbitrum paragraph, Citizens' House/Dual Governance paragraph, Fritsch et al. extension paragraph), abstract finding 4, Section 1.4 finding 4, and the Section 4.1 design-hypotheses synthesis to reflect this finding.

**On Lido.** The reviewer's observation that Lido's ratio "appears lower" with the revised holding HHI is opposite to what our data produces. The R1-corrected Lido holding HHI (Table 4 value 0.013) yields an amplification ratio of 6.8x, which is higher than the R1 Table 7's reported 4.8x. The R2 Table 7 reports 6.8x. If the reviewer is computing a different Lido holding HHI from an alternative exclusion list (for example, one that does not exclude the Lido DAO Aragon treasury), we would welcome specification of the methodology; the canonical exclusion set we apply is documented in Supplementary File S6.
