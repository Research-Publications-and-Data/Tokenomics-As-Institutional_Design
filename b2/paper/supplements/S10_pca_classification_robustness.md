...
**Spec A (canonical 5-class).** Mann-Whitney p = 0.011, Cohen's d = 1.05. This was the prior of-record specification; it applied the Class 3 staking-aggregation exclusion inconsistently across sectors, so its d = 1.05 is reframed as an inflated estimate rather than the headline. The main text now reports the headline sector contrast under the voter-inclusive pass-through treatment (Cohen's d = 0.65, Mann-Whitney p = 0.028), with the consistent uniform staking-aggregation exclusion as a robustness check (Cohen's d = 0.75, p = 0.018), robustly significant across permutation, leave-one-out, and the properly-specified multivariate test (Section 4.6.2).
